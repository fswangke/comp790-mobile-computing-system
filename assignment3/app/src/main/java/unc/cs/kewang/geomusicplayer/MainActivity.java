...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks, LocationListener, ResultCallback<Status>, OnMapReadyCallback {
    ...
    private String mPreviousEnteredPlace;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (Map.Entry<String, LatLng> entry : Constants.UNC_LANDMARKS_GPS.entrySet()) {
            mGeofenceList.add(new Geofence.Builder()
                    .setRequestId(entry.getKey())
                    .setCircularRegion(entry.getValue().latitude, entry.getValue().longitude, Constants.UNC_LANDMARKS_RADIUS.get(entry.getKey()))
...
//        mGoogleMap.addCircle(new CircleOptions().center(Constants.UNC_LANDMARKS_GPS.get(Constants.SITTERSON_NAME)).radius(Constants.UNC_LANDMARKS_RADIUS.get(Constants.SITTERSON_NAME)).strokeWidth(5).strokeColor(Color.RED));
//        mGoogleMap.addCircle(new CircleOptions().center(Constants.UNC_LANDMARKS_GPS.get(Constants.POLK_PLACE_NAME)).radius(Constants.UNC_LANDMARKS_RADIUS.get(Constants.POLK_PLACE_NAME)).strokeWidth(5).strokeColor(Color.RED));
//        mGoogleMap.addCircle(new CircleOptions().center(Constants.UNC_LANDMARKS_GPS.get(Constants.OLD_WELL_NAME)).radius(Constants.UNC_LANDMARKS_RADIUS.get(Constants.OLD_WELL_NAME)).strokeWidth(5).strokeColor(Color.RED));
    }
    ...
    class GeofenceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (transitionType.equals(Constants.GEOFENCE_TRANSITION_EXIT)) {
                ...
                if (placeName.equals(mPreviousEnteredPlace)) {
                ...
                mPreviousEnteredPlace = placeName;
                ...
                LatLng center = Constants.UNC_LANDMARKS_GPS.get(placeName);
                mGeofenceCircle = mGoogleMap.addCircle(new CircleOptions().center(center).radius(Constants.UNC_LANDMARKS_RADIUS.get(placeName)).strokeWidth(5).strokeColor(Color.RED));
            }
        }
    }
    ...
    class GeocoderBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (status == Constants.GEOCODER_SUCCESS_RESULT) {
                ...
                mAddressTextView.setText(errorMessage);
            }
        }
    }
}
